...
def pre_process(task):
    ...
    model = net.InpaintGenerator(task).to(device)
...
def get_inpaint_mode_for_delogo(H, W, mask):  # get inpaint area
    mode = []
    if H <= W:
        if not np.all(mask[:H, :H] == 0):
            mode.append('left')
        if not np.all(mask[:H, -H:] == 0):
            mode.append('right')
    else:
        if not np.all(mask[:W, :W] == 0):
            mode.append('top')
        if not np.all(mask[-W:, :W] == 0):
            mode.append('bottom')
    return mode

def main_for_delogo():  # delogo
    # set up models
    w, h = 360, 360
    clip_gap, device, frame_info, mask, model, reader, rec_time, video_name, writer = pre_process(args.task)

    square = min(frame_info['H_ori'], frame_info['W_ori'])
    mode = get_inpaint_mode_for_delogo(frame_info['H_ori'], frame_info['W_ori'], mask)
    ...
    for i in range(rec_time):
        ...
        for j in range(start_f, end_f):
            ...
            if 'left' in mode or 'top' in mode:
                image_crop1 = image[:square, :square, :]
                image_resize1 = cv2.resize(image_crop1, (w, h))
                frames1.append(image_resize1)
            if 'right' in mode or 'bottom' in mode:
                if 'right' in mode:
                    image_crop2 = image[:square, -square:, :]
                else:
                    image_crop2 = image[-square:, :square, :]
                image_resize2 = cv2.resize(image_crop2, (w, h))
                frames2.append(image_resize2)
        if 'left' in mode or 'top' in mode:
            comps1 = process(frames1, model, device, w, h)
        if 'right' in mode or 'bottom' in mode:
        ...
        if mode is not []:
            for j in range(end_f-start_f):
                ...
                if 'right' in mode or 'bottom' in mode:
                    comp = cv2.resize(comps2[j], (square, square))
                    comp = cv2.cvtColor(np.array(comp).astype(np.uint8), cv2.COLOR_BGR2RGB)
                    if 'right' in mode:
                        mask_area = mask[:square, -square:, :]
                        frame[:square, -square:, :] = mask_area * comp + (1 - mask_area) * frame[:square, -square:, :]
